...
local sr_window_imgbox = screen_rec_ui.screen_rec_window_imgbox
local sr_window_button = screen_rec_ui.screen_rec_window_button
...
if status_audio then
    sr_audio_button.bg = "#EE4F8466"
else
    sr_audio_button.bg = beautiful.groups_bg
end

if status_window then
    sr_window_button.bg = "#EE4F84" .. "66"
else
    sr_window_button.bg = beautiful.groups_bg
end
...
local sr_audio_mode = function()
	...
	-- Set window recording mode
local sr_window_mode = function()
    if not status_recording and not status_countdown then
        if status_window then
            status_window = false
            sr_window_button.bg = beautiful.groups_bg
...
sr_close_button:buttons(awful.button({}, 1, nil, screen_rec_close))

-- Right click to exit
local screen_close_on_rmb = function(widget)
    widget:buttons(awful.button({}, 3, nil, screen_rec_close))
end
...
local sr_recording_start = function()
    ...
    sr_start_recording(status_audio, status_window)
end
...
sr_audio_button:buttons(awful.button({}, 1, nil, sr_audio_mode))
sr_window_button:buttons(awful.button({}, 1, nil, sr_window_mode))
...
sr_main_button:buttons(awful.button({}, 1, nil, status_checker))
